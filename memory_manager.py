...
def save_memory(chat_id: int | str, memory: dict) -> None:
...
def update_memory(chat_id: int | str, memory_update: dict) -> dict:
    if not isinstance(memory_update, dict) or not memory_update:
        return load_memory(chat_id)
    memory = load_memory(chat_id)
    if _recursive_update(memory, memory_update):
        save_memory(chat_id, memory)
        print(f"[Memory] Saved for {chat_id}: {list(memory_update.keys())}")
    return memory
